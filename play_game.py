...
class PlayGame:
    def __init__(self, screen, mode='singleplayer', size=10):
        ...
        self.renderer = GameRenderer(screen, self.cell_size, (self.screen_width - self.width * self.cell_size) // 2, (self.screen_height - self.height * self.cell_size) // 2, self.width, self.height)
        
    def update_game_elements(self):
        self.ui.update_dimensions()
        self.renderer.update_offset((self.screen_width - self.width * self.cell_size) // 2, (self.screen_height - self.height * self.cell_size) // 2)
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self.ui.back_button.click(event):
                    self.return_to_menu = True
                # Check if the click is on the slider
                elif self.ui.speed_slider.rect.collidepoint(event.pos):
                    relative_x = event.pos[0] - self.ui.speed_slider.rect.left
                    percentage = relative_x / self.ui.speed_slider.rect.width
                    new_speed = percentage * (self.ui.speed_slider.value_range[1] - self.ui.speed_slider.value_range[0]) + self.ui.speed_slider.value_range[0]
                    self.ui.speed_slider.set_current_value(new_speed)
    ...
    def draw(self):
        ...
        self.renderer.draw_snake(self.snake, self.last_move)
